...
const uploadResume = async (event) => {
    ...
    try {
        ...
        console.log('📤 Uploading resume text:', resumeText.length, 'chars');
  ...
  return (
    <div>
        <div className='max-w-7xl mx-auto px-4 py-8'>
          ...
          text-transparent sm:hidden'>Welcome , venkat</p>
          ...
          <div className='flex gap-4'>
            ...
            gap-2 text-slate-600 text-slate-600 border border-dashed border-slate-300 group hover:border-indigo-500 hover:shadow-lg
            transition-all duration-300 cursor-pointer'>
                <PlusIcon className='size-11 transition-all duration-300 p-2.5 bg-gradient-to-br from-indigo-500
            ...
            gap-2 text-slate-600 text-slate-600 border border-dashed border-slate-300 group hover:border-purple-500 hover:shadow-lg
            transition-all duration-300 cursor-pointer'>
                <UploadCloudIcon className='size-11 transition-all duration-300 p-2.5 bg-gradient-to-br from-purple-500
            text-white rounded-full'/>
             ...
             {allResumes.map((resume,index)=>{
                  ...
                  return (
                    ...
                    style={{background:`linear-gradient(135deg,${baseColor}10,${baseColor}40)`,borderColor:baseColor+'40'}}>
                     ...
                     <p className='absolute bottom-1 text-[11px] text-slate-800 group-hover:text-salte-700 transition-all duration-all duration-300 px-2 text-center'
                     style={{color:baseColor+'90'}}>Updated on {new Date(resume.updatedAt).toLocaleDateString()}</p>

                     <div onClick={(e)=>e.stopPropagation()} className='absolute top-1 right-1 group-hover:flex items-center hidden'>
                      <TrashIcon onClick={()=>deleteResume(resume._id)} className='size-7 p-1.5 hover:bg-color-white/50 rounded text-slate-700 transition-colors'/>
                      <PencilIcon onClick={()=>{setEditResumeId(resume._id);setTitle(resume.title)}} className='size-7 p-1.5 hover:bg-color-white/50 rounded text-slate-700 transition-colors'/>
                     </div>
                    ...
                    <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                        ...
                        <div>
                            <label htmlFor="resume-input" className='block text-sm text-slate-700'>
                                Select resume file
                                <div className='flex flex-col items-center jusrify-center gap-2 border group text-slate-400
                            ...
                            <input type="file" id='resume-input' accept='' hidden onChange={(e)=>setResume(e.target.files[0])} />
